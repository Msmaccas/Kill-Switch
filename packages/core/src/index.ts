...
function genId(): string {
  return (
    'thesis-' + Math.random().toString(36).slice(2, 10)
  );
}
import {
  Thesis,
 ...
 */
export function createThesis(params: Omit<Thesis, 'id' | 'createdAt' | 'lastReviewedAt'>): Thesis {
 ...
 */
export function evaluateThesis(thesis: Thesis, events: Event[]): DriftEvent[] {
  ...
  for (const ev of events) {
    if (ev.type === 'price') {
      const change = (ev.attributes['percentageChange'] as number) ?? 0;
...
export function maxSeverity(events: DriftEvent[]): BreakSeverity {
  let severity: BreakSeverity = 'NONE';
  for (const ev of events) {
    if (ev.severity === 'HARD') {
      return 'HARD';
    }
...
export function isReviewDue(thesis: Thesis, now: Date): boolean {
 ...
 */
export function buildBoardItem(thesis: Thesis, driftEvents: DriftEvent[], now: Date): BoardItem {
